...
(function(){var Books,libr;libr=angular.module("libr.services.books",["ngResource"]),Books=function(){function Books($resource,Constant){var url;return this.$resource=$resource,this.Constant=Constant,url=this.Constant.baseUrl+"/books/:book_id",this.$resource(url,{user_email:localStorage.getItem("email"),user_token:localStorage.getItem("token")},{query:{method:"GET",isArray:!1,cache:!1},fetchNew:{method:"GET",isArray:!1,cache:!1,url:this.Constant.baseUrl+"/books/newbooks/:afterId"}})}return Books.$inject=["$http","$resource","Constant"],Books}(),libr.factory("Books",["$resource","Constant",Books])}).call(this);
...
(function(){var DoubanService,libr,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}};libr=angular.module("libr.services.douban",[]),DoubanService=function(){function DoubanService($http,Constant){this.$http=$http,this.Constant=Constant,this.submitUser=__bind(this.submitUser,this)}return DoubanService.$inject=["$http","Constant"],DoubanService.prototype.userInfo=function(user,callback,error){return this.$http({url:"http://api.douban.com/v2/user/"+user,method:"GET",timeout:1e4}).success(function(data,status){return 200===status?callback(data):error(data)}).error(function(data){return error(data)})},DoubanService.prototype.submitUser=function(user,callback,error){var email,token;return email=localStorage.getItem("email"),token=localStorage.getItem("token"),this.$http({url:""+this.Constant.baseUrl+"/link/douban/"+user+"?user_email="+email+"&user_token="+token,method:"POST",timeout:1e4}).success(function(data){return callback(data)}).error(function(data){return error(data)})},DoubanService}(),libr.service("DoubanService",DoubanService)}).call(this);

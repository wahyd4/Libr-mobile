...
(function(){var AuthicationService,libr,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}};libr=angular.module("libr.services.auth",[]),AuthicationService=function(){function AuthicationService($http){this.$http=$http,this.register=__bind(this.register,this),this.login=__bind(this.login,this),console.log("init auth service")}return AuthicationService.$inject=["$http"],AuthicationService.prototype.login=function(user,callback,errorCallback){var baseUrl;return baseUrl="http://libr.herokuapp.com/api/v1/sessions",this.$http({method:"POST",url:baseUrl,data:"user[email]="+user.email+"&user[password]="+user.password,headers:{"Content-Type":"application/x-www-form-urlencoded"}}).success(function(result){return result.success?callback(result):errorCallback("登录失败,请输入正确的用户名和密码")}).error(function(){return errorCallback("登录失败,请输入正确的用户名和密码")})},AuthicationService.prototype.register=function(user,callback,onError){var url;return url="http://libr.herokuapp.com/api/v1/registrations",this.$http({method:"POST",url:url,data:"user[email]="+user.email+"&user[password]="+user.password,headers:{"Content-Type":"application/x-www-form-urlencoded"}}).success(function(result,status){return callback(result,status)}).error(function(data){return onError(data)})},AuthicationService}(),libr.service("AuthService",AuthicationService)}).call(this);
(function(){var Books,libr;libr=angular.module("libr.services.books",["ngResource"]),Books=function(){function Books($resource){var baseUrl;return this.$resource=$resource,baseUrl="http://libr.herokuapp.com/api/v1/books/:book_id",this.$resource(baseUrl,{user_email:localStorage.getItem("email"),user_token:localStorage.getItem("token")},{query:{method:"GET",isArray:!1,cache:!1},fetchNew:{method:"GET",isArray:!1,cache:!1,url:"http://libr.herokuapp.com/api/v1/books/newbooks/:afterId"}})}return Books.$inject=["$http","$resource"],Books}(),libr.factory("Books",["$resource",Books])}).call(this);
(function(){var CommentService,libr;libr=angular.module("libr.services.comment",["ngResource"]),CommentService=function(){function CommentService($resource){var baseUrl;return this.$resource=$resource,baseUrl="http://libr.herokuapp.com/api/v1/books/:book_id/comments/:comment_id",this.$resource(baseUrl,{user_email:localStorage.getItem("email"),user_token:localStorage.getItem("token"),book_id:"@bookId",comment_id:"@commentId"},{query:{method:"GET",isArray:!0,cache:!1}},{save:{params:{comment_id:"@commentId"}}})}return CommentService.$inject=["$http","$resource"],CommentService}(),libr.factory("Comments",["$resource",CommentService])}).call(this);
(function(){var DoubanService,libr;libr=angular.module("libr.services.douban",[]),DoubanService=function(){function DoubanService($http){this.$http=$http}return DoubanService.$inject=["$http"],DoubanService.prototype.userInfo=function(user,callback,error){return this.$http({url:"http://api.douban.com/v2/user/"+user,method:"GET",timeout:1e4}).success(function(data,status){return 200===status?callback(data):error(data)}).error(function(data){return error(data)})},DoubanService.prototype.submitUser=function(user,callback,error){var email,token;return email=localStorage.getItem("email"),token=localStorage.getItem("token"),this.$http({url:"http://libr.herokuapp.com/api/v1/link/douban/"+user+"?user_email="+email+"&user_token="+token,method:"POST",timeout:1e4}).success(function(data){return callback(data)}).error(function(data){return error(data)})},DoubanService}(),libr.service("DoubanService",DoubanService)}).call(this);
